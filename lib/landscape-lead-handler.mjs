...
const FIELD_LIMITS = {
  ticket_id: 128,
  form_type: 32,
  "cf-turnstile-response": TURNSTILE_MAX_TOKEN_LENGTH,
  full_name: 120,
  first_name: 80,
  last_name: 80,
  email: 254,
...
async function verifyTurnstile({ data, env, expectedHostname, expectedAction, remoteIp, fetchImpl }) {
  const secret = configValue(env, "TURNSTILE_SECRET_KEY");
  if (!secret) {
    throw new LeadRequestError(503, "Security verification is temporarily unavailable. Please call us directly.");
  }

  const token = valueFrom(data, ["cf-turnstile-response"]);
  if (!token || token.length > TURNSTILE_MAX_TOKEN_LENGTH) {
    throw new LeadRequestError(422, "Please complete the security check and try again.");
  }
  ...
  const errorCodes = Array.isArray(result && result["error-codes"])
    ? result["error-codes"].map((value) => cleanValue(value))
    : [];
  if (!result || result.success !== true) {
    const configurationFailure = errorCodes.some((code) => (
    ...
    throw new LeadRequestError(
      ...
      configurationFailure
        ? "Security verification is temporarily unavailable. Please call us directly."
        : "Please complete the security check and try again."
    );
  }

  if (result.hostname !== expectedHostname || result.action !== expectedAction) {
    throw new LeadRequestError(403, "Please complete the security check and try again.");
  }
}
...
async function sendToGoogleSheets(normalized, env, meta, fetchImpl) {
  ...
  if (!payload || payload.ok !== true) {
    throw new Error(`Google Sheets webhook error: ${safeText(payload && payload.error, "webhook did not confirm storage")}`);
  }
  return {
    ok: true,
...
export async function handleLeadRequest({
  ...
  try {
    ...
    const { formType } = validateLeadPayload(data, now);
    await verifyTurnstile({
      data,
      env,
      expectedHostname: requestUrl.hostname,
      expectedAction: formType,
      remoteIp: cleanValue(request.headers.get("cf-connecting-ip")).slice(0, 64),
      fetchImpl
    });
